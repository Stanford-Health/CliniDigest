...
def output_files(filename, medical_fields):
    with open(filename, encoding = 'utf-8') as f:
        ...
        for medical_field in medical_fields:
            ...
            device = re.match(f'{dir}devices_master_lists/(.*).json', filename).group(1)
            token_length = num_tokens_from_string(output, 'cl100k_base')
            if token_length < 11250:
                with open(f'{dir}gpt_code/internal_files/devices_gpt_input/{device}_{medical_field}.txt', 'w') as outfile:
                ...
                for i in range(1, num_files + 1):
                    with open(f'{dir}gpt_code/internal_files/devices_gpt_input_files/{device}_{medical_field}{i}.txt', 
                        ...
                        if index >= key_trials:
                            ...
                            #end = output.index(str(index) + '. Title: ')
                        temp = output[start:end]
                        outfile.write(output[start:min(end, len(output))])
                        start = end
            with open(f'{dir}gpt_code/internal_files/devices_references/{device}_{medical_field}.txt', 'w') as outfile:
                    outfile.writelines('\n'.join(reference_list))
...
def main():
    ...
    with open(f'{dir}gpt_code/internal_files/{filename1}', encoding = 'utf-8') as f:
        data = json.load(f)
        for device in data:
            medical_fields = data[device]
            output_files(f'{dir}gpt_code/devices_master_lists/{device}.json', medical_fields)
